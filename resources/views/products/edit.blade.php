                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" required
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="hostingaccount" {{ old('type', $product->type) === 'hostingaccount' ? 'selected' : '' }}>Hosting Account</option>
                                    <option value="domain" {{ old('type', $product->type) === 'domain' ? 'selected' : '' }}>Domain</option>
                                    <option value="addon" {{ old('type', $product->type) === 'addon' ? 'selected' : '' }}>Addon</option>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required
                                    ...
                                    <option value="Terminated" {{ old('status', $product->status) === 'Terminated' ? 'selected' : '' }}>Terminated</option>
                                    <option value="Cancelled" {{ old('status', $product->status) === 'Cancelled' ? 'selected' : '' }}>Cancelled</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="billing_cycle" id="billing_cycle" required
                                    ...
                                    <option value="quarterly" {{ old('billing_cycle', $product->billing_cycle) === 'quarterly' ? 'selected' : '' }}>Quarterly</option>
                                    <option value="semiannually" {{ old('billing_cycle', $product->billing_cycle) === 'semiannually' ? 'selected' : '' }}>Semi-Annually</option>
                                    <option value="annually" {{ old('billing_cycle', $product->billing_cycle) === 'annually' ? 'selected' : '' }}>Annually</option>
                                    <option value="biennially" {{ old('billing_cycle', $product->billing_cycle) === 'biennially' ? 'selected' : '' }}>Biennially</option>
                                    <option value="triennially" {{ old('billing_cycle', $product->billing_cycle) === 'triennially' ? 'selected' : '' }}>Triennially</option>
                                    <option value="onetime" {{ old('billing_cycle', $product->billing_cycle) === 'onetime' ? 'selected' : '' }}>One Time</option>
                                </select>
                                @error('billing_cycle')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
