        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex justify-between items-start">
                            ...
                            <div class="text-right">
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('messages.msg_invoice_date_label') }}</p>
                    ...
                    <!-- Client Information -->
                    <div class="mb-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                        <h4 class="text-lg font-semibold mb-3">{{ __('messages.msg_bill_to') }}</h4>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">{{ __('messages.msg_invoice_details') }}</h4>
                            <dl class="space-y-2">
                                <div class="flex justify-between">
                                    <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_invoice_number') }}</dt>
                                    <dd class="text-sm font-medium">{{ $invoice->invoice_number }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_invoice_date') }}</dt>
                                    <dd class="text-sm font-medium">{{ $invoice->date->format('M d, Y') }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_due_date') }}</dt>
                                    <dd class="text-sm font-medium">{{ $invoice->duedate->format('M d, Y') }}</dd>
                                </div>
                                @if($invoice->payment_method)
                                    <div class="flex justify-between">
                                        <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_payment_method') }}</dt>
                        ...
                        <div>
                            <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">{{ __('messages.msg_amount_details') }}</h4>
                            <dl class="space-y-2">
                                <div class="flex justify-between">
                                    <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_subtotal') }}</dt>
                                    <dd class="text-sm font-medium">{{ formatCurrencyVND($invoice->subtotal) }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_tax') }}</dt>
                                    <dd class="text-sm font-medium">{{ formatCurrencyVND($invoice->tax) }}</dd>
                                </div>
                                @if($invoice->credit > 0)
                                    <div class="flex justify-between">
                                        <dt class="text-sm text-gray-600 dark:text-gray-400">{{ __('messages.msg_credit') }}</dt>
                                        <dd class="text-sm font-medium text-green-600 dark:text-green-400">-{{ formatCurrencyVND($invoice->credit) }}</dd>
                                    </div>
                                @endif
                                <div class="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                                    <dt class="text-base font-semibold">{{ __('messages.msg_total') }}</dt>
                    ...
                    @if($invoice->notes)
                        <div class="mb-6">
                            <h4 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">{{ __('messages.msg_notes') }}</h4>
                            <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-md">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <div class="text-xs text-gray-500 dark:text-gray-400 space-y-1">
                            <p>{{ __('messages.msg_created') }} {{ $invoice->created_at->format('M d, Y h:i A') }}</p>
                            <p>{{ __('messages.msg_last_updated') }} {{ $invoice->updated_at->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>
                </div>
            </div>
